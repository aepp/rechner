...
//	    .wrap('<div class="" id="datetimepicker" data-date-format="dd.m.yyyy"></div>')
    ...
    $('.save-changes').unbind('click').click(function(event) {
        ...
        var produktId = $('.produktId').val(),
                ...
                sn = $('#produktInformationen').parent().find('div').eq(0).find('.note-editable').code();
        ...
        if ($('#produkt-form').validationEngine('validate')) {
            $.ajax({
                ...
                success: function(response) {
                    if (!response.error) {
                        window.location = response.redirect;	/* Redirect zur Übersicht bei Erfolg */
                    }
                    else {
    ...
    $('.konditionen-bearbeiten').unbind('click').click(function(event) {
        ...
        if (erste_schwelle.length > 0)
            erste_schwelle.tab('show');
    });
    ...
    function load_konditionen() {
        ...
        $('input[name="produktIsBonitabh-helper"]').each(function(i, helper) {
            if ($(helper).val() == bonitabh.val())
                $(this).prop('checked', bonitabh.prop('checked'));
        });
...
//				'<table class="table table-hover table-condensed" id="konditionen-table">'+
            ...
            $.each(risikoklassen[schwellen[i]], function(j, risikoklasse) {
                ...
                $.each(laufzeiten[schwellen[i]], function(k, laufzeit) {
                    if (!init)
                        zinssatz = zinssaetze[schwellen[i]][risikoklasse][laufzeit];
                    if (zinssatz != null)
                        zinssatz = zinssatz.toString().replace('.', ',');
                    table +=
            ...
            $.each(laufzeiten[schwellen[i]], function(j, laufzeit) {
                if (!init)
                    lead = leads[schwellen[i]][laufzeit];
                if (lead != null)
                    lead = lead.toString().replace('.', ',');
                table +=
            ...
            $.each(laufzeiten[schwellen[i]], function(j, laufzeit) {
                if (!init)
                    sale = sales[schwellen[i]][laufzeit];
                if (sale != null)
...
//        	$('.table-responsive').html('Wait...');
        ...
        if ($('.table-responsive').data[newValueName])
            $('.table-responsive').html($('.table-responsive').data[newValueName]);
        else {
...
//    	var schwellenIndex = $(this).parent().parent().parent().parent().parent().attr('id').split('-')[2];
        ...
        if (schwelleHeading.length == 0)
            table.find('thead').find('tr').eq(0).append('<th colspan="1"></th>');
        else
            schwelleHeading.attr('colspan', parseInt(schwelleColspan) + 1);
        ...
        table.find('thead').find('tr').each(function(i, tr) {
            if (i != 0 && i != 1)
                $(tr).find('th').eq(colIndex).after(thead[i]);
        });
    ...
    $(document).on('click', '.remove-laufzeit', function(e) {
        ...
        if (schwelleColspan == 1)
            schwelleHeading.remove();
        else
            schwelleHeading.attr('colspan', parseInt(schwelleColspan) - 1);

        table.find('thead').find('tr').each(function(i, tr) {
            if ($(tr).find('th').eq(colIndex).find('input').val())
                inputChanged = true;
            if (i != 0 && i != 1)
        ...
        table.find('tbody').find('tr').each(function(i, tr) {
            if ($(tr).find('td').eq(colIndex + 2).find('input').val())
                inputChanged = true;
    ...
    $(document).on('click', '.remove-risikoklasse', function(e) {
        ...
        if (table.find('tbody tr').length == 1)
            table.find('tbody tr').find('td').eq(1).find('button').addClass('disabled');
    });
...
//    	var div_tab_content = $(this).parent().parent().parent().parent().parent().parent();
        ...
        if (tab_link.length == 0)
            tab_link = $('.nav-tabs').find('li').eq(div_tab_index - 1);
